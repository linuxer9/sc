...
def open_search(widget, username):
	os.system("/usr/lib/sc/mintInstall.py " + username + " &")

def open_featured(widget):
	gladefile = "/usr/lib/sc/frontend.glade"
	...
	wTree.get_widget("featured_window").set_icon_from_file("/usr/lib/sc/icon.svg")	
...
def show_screenshot(widget, model):
	...
	if model.selected_application != None:		
		...
		wTree.get_widget("screenshot_window").set_icon_from_file("/usr/lib/sc/icon.svg")
...
def show_more_info(widget, model):
	if model.selected_application != None:
		if not os.path.exists((model.selected_application.mint_file)):			
			...
			wTree.get_widget("more_info_window").set_icon_from_file("/usr/lib/sc/icon.svg")
...
def install(widget, model, wTree, username):	
	if model.selected_application != None:
		if model.selected_application.is_special:
			...
			model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/lib/sc/status-icons/add.png"))		
	...
	if model.selected_application != None:
		if model.selected_application.is_special:
			if not os.path.exists((model.selected_application.mint_file)):
				...
				os.system("/usr/lib/sc/remove.py " + model.selected_application.mint_file)
				show_item(wTree.get_widget("tree_applications").get_selection(), model, wTree, username)
				global cache
				cache = apt.Cache()
				show_applications(wTree, model, False)
			...
			wTree.get_widget("toolbutton_apply").set_sensitive(True)
			model_applications, iter = wTree.get_widget("tree_applications").get_selection().get_selected()			
			model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/lib/sc/status-icons/remove.png"))		
...
def cancel_change(widget, model, wTree, username):
	if model.selected_application != None:
		...
		if model.selected_application.status == "add":
			...
			model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/lib/sc/status-icons/available.png"))		
			...
			model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/lib/sc/status-icons/installed.png"))		
		...
		for item in portal.items:	
			if (item.category.key in category_keys):
				if item.status in matching_statuses and (model.keyword == None 
					...
					if item.is_special:
						model_applications.set_value(iter, 7, gtk.gdk.pixbuf_new_from_file("/usr/lib/sc/status-icons/special.png"))	
...
class RefreshThread(threading.Thread):
		...
		try:
			...
			try:
				...
				if allPortalsHere:
					...
					dialog.set_icon_from_file("/usr/lib/sc/icon.svg")
	...
	def load_model_in_GUI(self, wTree, model):
		...
		model_categories.set_value(iter, 2, gtk.gdk.pixbuf_new_from_file_at_size("/usr/lib/sc/icon.svg", 16, 16))
		for portal in model.portals:
			for category in portal.categories:		
